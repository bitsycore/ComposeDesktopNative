...
// tools/build-*.sh, which only runs on Windows). Declaring `mingwX64()` on
// a non-Windows host is safe for pure-Kotlin modules but blows up the moment
// a `depends = sdl3` cinterop tries to include SDL3_ttf/SDL_ttf.h. Blocking
// the target here also fixes root-metadata publish: the KotlinMultiplatform
// publication skips when a per-target metadata task fails, and macOS-CI
// couldn't emit mingw metadata → no `.module` JSON → consumers can't resolve
// the common coord `desktop-window:X` (they see only the per-target ones).
