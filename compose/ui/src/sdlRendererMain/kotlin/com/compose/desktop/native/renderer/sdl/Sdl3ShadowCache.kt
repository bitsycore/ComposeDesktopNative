...
internal class Sdl3ShadowCache(private val fRenderer: COpaquePointer) {
	...
	fun getGeneric(inPath: androidx.compose.ui.graphics.Path, inBlurPx: Int): GenericEntry? {
		val vProject = inPath as? com.compose.desktop.native.graphics.ProjectPath ?: return null
	...
	private fun geometryHash(inPath: com.compose.desktop.native.graphics.ProjectPath): Long {
		var vHash = 1125899906842597L
		fun fold(inV: Float) { vHash = vHash * 31 + inV.toRawBits() }
		for (vCmd in inPath.commands) when (vCmd) {
			is com.compose.desktop.native.graphics.PathCommand.MoveTo -> { fold(1f); fold(vCmd.x); fold(vCmd.y) }
			is com.compose.desktop.native.graphics.PathCommand.LineTo -> { fold(2f); fold(vCmd.x); fold(vCmd.y) }
			is com.compose.desktop.native.graphics.PathCommand.QuadTo -> { fold(3f); fold(vCmd.cx); fold(vCmd.cy); fold(vCmd.x); fold(vCmd.y) }
			is com.compose.desktop.native.graphics.PathCommand.CubicTo -> { fold(4f); fold(vCmd.c1x); fold(vCmd.c1y); fold(vCmd.c2x); fold(vCmd.c2y); fold(vCmd.x); fold(vCmd.y) }
			com.compose.desktop.native.graphics.PathCommand.Close -> fold(5f)
		}
		return vHash
	}

	private fun buildGeneric(inPath: com.compose.desktop.native.graphics.ProjectPath, inBlur: Int): GenericEntry? {
	...
	private fun flattenPath(inPath: com.compose.desktop.native.graphics.ProjectPath): List<FloatArray> {
		...
		for (vCmd in inPath.commands) when (vCmd) {
			is com.compose.desktop.native.graphics.PathCommand.MoveTo -> {
				closeCurrent()
				vCx = vCmd.x; vCy = vCmd.y
				vCurrent.add(vCx); vCurrent.add(vCy)
			}
			is com.compose.desktop.native.graphics.PathCommand.LineTo -> {
				vCx = vCmd.x; vCy = vCmd.y
				vCurrent.add(vCx); vCurrent.add(vCy)
			}
			is com.compose.desktop.native.graphics.PathCommand.QuadTo -> {
				for (vI in 1..12) {
			...
			is com.compose.desktop.native.graphics.PathCommand.CubicTo -> {
				for (vI in 1..16) {
			...
			com.compose.desktop.native.graphics.PathCommand.Close -> closeCurrent()
		}
		closeCurrent()
		return vContours
	}
