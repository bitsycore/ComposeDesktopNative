...
## ✅ PARALLEL-WORLD DELETION (2026-07-02) — ProjectLayoutNode is gone

Deleted the two-worlds bridge — the project's own `ProjectLayoutNode` runtime tree and every
adapter that fed it. Nothing outside its own adapter cluster referenced these types after B4/B5/B6a
(verified via grep). Runtime screenshot (`--screen=Buttons`) is byte-identical before/after
(`ce15decb83c3bb7ba44660cd9002408c`).

**~2600 lines removed** across these commits:
- `Sdl3Renderer.kt` (782L) + `SkiaRenderer.kt` (356L) — legacy tree walkers.
- `ProjectLayoutNode.kt` (1142L) — the class itself + its measure/hit-test pipeline.
- `ProjectNodeChain.kt` (172L) — per-project-node Modifier.Node chain.
- `LayoutNodeMeasurable.kt` + `LayoutPolicyAdapter.kt` + `LayoutNodePlaceable` — measure adapters.
- `InternalMeasurePolicy.shim.kt` + `ListForEachReversed.shim.kt` — bridging shims.
- `RenderBackend.draw(ProjectLayoutNode)` interface method + both implementations.
- Legacy `ImageMeasurePolicy` + `TextMeasurePolicy` (unreachable — Image/BasicText already on
  upstream `Layout` + `Text/PainterDrawElement`).
- `WrappedContentDrawScope.kt` — unused Phase-8 chain wrapper.
- Vendored `ComposeUiNode.kt` (verbatim from project) + `LayoutModifier.kt` (283L upstream
  replaces project shim + LayoutModifierElement/Impl).
- Decoupled `FocusRequester`/`FocusManager` types (ProjectLayoutNode → `Any`) so focus can
  live on the upstream engine later without a type migration.

**Current state (2026-07-02):**
- `commonMain`: **91 files** (was 100) — 62 non-shim + 29 shim + project glue.
- `nativeMain`: **48 files** — SDL3 windowing/event actual + Path/Paint/Canvas natives.
- `vendor/`: **593 files** — regenerated by `sync.sh` from cmp-ref.
- Buttons screenshot: `ce15decb83c3bb7ba44660cd9002408c` (unchanged across all deletions).
- Full mingw + macOS-sdl3 graph compiles green; macOS-skia is compile-only (SkiaCanvas actual
